...
Invader.prototype = {
  ...
  audio_buffers: {},
  sounds: {
    0: 'https://s3.amazonaws.com/all-day/INVADER-LV1.wav',
    1: 'https://s3.amazonaws.com/all-day/INVADER-LV2.wav',
    2: 'https://s3.amazonaws.com/all-day/INVADER-LV3.wav',
    3: 'https://s3.amazonaws.com/all-day/INVADER-LV4.wav'
  },
  current_sound: null,
  sound_interval: 15360,
  original_sound_interval: 15360,
  change_playrate: false,
  playrate_increment: 0.05,
  next_sound: 0,
  ...
  createAudioContext: function(buffer, game_sound) {
      ...
      if (!game_sound) {
        ...
        if (this.change_playrate) {
          sound_source.playbackRate.value = 1.00 + (this.kills - this.size(this.sounds) + 1) * this.playrate_increment;
          this.change_playrate = false;
          clearInterval(this.sound_interval_object);
          this.sound_interval -= (this.original_sound_interval) * (this.kills - this.size(this.sounds) + 1) * this.playrate_increment;
          this.observeSound();
        }
  ...
  playSound: function(name) {
      this.audio_buffers[name].sound_source.start(this.audio_buffers[name].audio_context.currentTime);
  ...
  updateStats: function() {
    ...
    for(var n = 0; n <= this.columns; n++) {
        ...
        if (item != undefined && stats_n >= item.length) {
            $colon = this.createElement('div', 'item stats', ':');
            $value = this.createElement('div', 'item stats', this[item]);
            $spacing1 = this.createElement('div', 'item', '&nbsp;');
            $spacing2 = this.createElement('div', 'item', '&nbsp;');
            $spacing3 = this.createElement('div', 'item', '&nbsp;');
            $spacing4 = this.createElement('div', 'item', '&nbsp;');
